...
vim.keymap.set('n', 'j', "v:count == 0 ? 'gjzz' : 'jzz'",
	{ expr = true, silent = true, desc = 'Navigate Down (also in wrap mode)' })
vim.keymap.set('n', 'k', "v:count == 0 ? 'gkzz' : 'kzz'",
	{ expr = true, silent = true, desc = 'Navigate Up (also in wrap mode)' })
...
vim.keymap.set({ "n", "v" }, "<leader>D", [["*D]],
	{ desc = 'Delete the characters under the cursor until the end of the line and [count]-1 more lines (register *)' })

-- copy current buffer fullpath into clipboard
vim.keymap.set('n', '<leader>wb', function()
	local filepath = vim.fn.expand('%:p')
	vim.fn.setreg('*', filepath)
	vim.fn.system("tmux display-message \"Copied '" .. filepath .. "' into clipboard!\"")
end, { desc = 'Copy full path of current buffer into clipboard' })
